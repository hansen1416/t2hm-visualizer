...
class AnimPlayer:
    ...
    def _init_smpl(self, gender: str = "male"):
        ...
        name = f"__body_model__"
        self._scene.scene.add_geometry(name, self.body_mesh, self.material)
    ...
    def _load_data(self, motion_path: str):
        ...
        motion_name, gender, beta, trans, root_orient, pose_body = (
            self.pager.load_single(motion_path)
        )
        ...
        self.num_frames = trans.shape[0]

        self.frame_idx = 0

        m_verts, m_joints = bm(
            poses_body=pose_body.reshape(self.num_frames, -1).to(self.device),
            betas=beta.to(self.device),
            poses_root=root_orient.to(self.device),
            trans=trans.to(self.device),
        )

        self.verts_glob = m_verts.cpu().numpy()
        # + self.offsets[mesh_idx]

        # # offset_height: per-sample scalar computed from frame-0 grounding
        # oh = offset_height[mesh_idx]

        # # make sure it's a python float (works if oh is torch scalar / 0-d np / python)
        # if torch.is_tensor(oh):
        #     oh = float(oh.detach().cpu().item())
        # elif isinstance(oh, np.ndarray):
    ...
    def animate_mesh(self):
        ...
        while True:
            ...
            verts_frame = self.verts_glob[frame_idx].copy()

            def update(verts_frame=verts_frame):
                # UI thread ONLY: touch Open3D objects here
                self.body_mesh.vertices = o3d.utility.Vector3dVector(verts_frame)
